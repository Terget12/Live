...
def show_user_ranges(chat_id, srv_id, cnt_id, message_id=None):
    ...
    text = f"{emo('number')} <b>SELECT RANGE</b> {emo('number')}\n━━━━━━━━━━━━━━━━━━\n{emo(srv_data['name'])} Service: <code>{html.escape(srv_data['name'])}</code>\n{get_flag(cnt_data['name'])} Country: <code>{html.escape(cnt_data['name'])}</code>\n\nTap a range to allocate a number:"
    safe_send(chat_id, text, markup, message_id)
...
def fetch_number(chat_id, service_info, msg_id, is_custom=False, retry=0):
    ...
    for _ in range(NUMBER_ALLOCATION_COUNT):
        ...
        for panel in active_panels:
            try:
                res = requests.post(f"{panel['url']}/api/v1/numbers/get", json=payload, headers={"X-API-Key": panel["key"]}, timeout=7)
                data = res.json()
    ...
    make_poll_state(chat_id, [num_data["number_id"] for num_data in final_numbers])

    for num_data in final_numbers:
        threading.Thread(target=poll_user_otp, args=(chat_id, num_data["number_id"], service_info, num_data["number"], msg_id, is_custom, num_data["panel_url"], num_data["panel_key"]), daemon=True).start()
...
def poll_user_otp(chat_id, num_id, service_info, allocated_number, msg_id, is_custom, panel_url, panel_key):
    ...
    while time.time() - start < OTP_POLL_TIMEOUT:
        ...
        if not poll_state.get("active", False) or not poll_state.get("numbers", {}).get(num_id, False):
            break

        try:
            r = requests.get(f"{panel_url}/api/v1/numbers/{num_id}/sms", headers={"X-API-Key": panel_key}, timeout=10).json()
            ...
            if r.get("success") and r.get("otp"):
                ...
                for grp in fwd_groups:
                    try:
                        ...
                        if HAS_COPY_BTN: otp_markup.add(InlineKeyboardButton(f"📋 {otp_code}", copy_text=CopyTextButton(text=str(otp_code))))
                        else: otp_markup.add(InlineKeyboardButton(f"📋 {otp_code}", callback_data=f"copyn|{otp_code}"))
                        
                        for b in grp.get("buttons", []):
                            if b.get("name") and b.get("url"): otp_markup.add(InlineKeyboardButton(b["name"], url=b["url"]))
...
def show_admin_panel(chat_id, message_id=None):
    ...
    text = f"{emo('crown')} <b>ADMIN PANEL</b> {emo('crown')}\n━━━━━━━━━━━━━━━━━━\n📈 <b>TODAY</b> ({today})\n🔢 Numbers: <code>{td['numbers']}</code>  🔐 OTPs: <code>{td['otps']}</code>\n━━━━━━━━━━━━━━━━━━\n🌍 <b>Total All-Time</b>\n🔢 Numbers: <code>{gs.get('total_numbers', 0)}</code>  🔐 OTPs: <code>{gs.get('total_otps', 0)}</code>\n━━━━━━━━━━━━━━━━━━"
...
def get_force_join_menu():
    ...
    for idx, link in enumerate(channels): markup.add(InlineKeyboardButton(f"❌ Remove: {link}", callback_data=f"delfjc_{idx}"))
...
def get_group_settings_menu():
    ...
    if otp_link and otp_link != "https://t.me/": markup.add(InlineKeyboardButton("🗑️ Remove OTP Link", callback_data="del_main_otp_link"))
...
def show_admin_services(chat_id, message_id=None):
    data = load_data()
    markup = InlineKeyboardMarkup(row_width=2)
    for srv_id, srv in data.get("services_data", {}).items(): markup.add(InlineKeyboardButton(text=f"📁 {srv['name']}", callback_data=f"adm_s|{srv_id}"))
...
def show_admin_countries(chat_id, srv_id, message_id=None):
    ...
    if not srv_data: return
    markup = InlineKeyboardMarkup(row_width=2)
    for cnt_id, cnt in srv_data.get("countries", {}).items(): markup.add(InlineKeyboardButton(text=f"{get_flag(cnt['name'])} {cnt['name']}", callback_data=f"adm_c|{srv_id}|{cnt_id}"))
...
def show_admin_ranges(chat_id, srv_id, cnt_id, message_id=None):
    ...
    flag   = get_flag(cnt_data["name"])
    markup = InlineKeyboardMarkup(row_width=1)
    for rng_id, rng_val in cnt_data.get("ranges", {}).items(): markup.add(InlineKeyboardButton(text=f"❌ {rng_val}", callback_data=f"del_rng|{srv_id}|{cnt_id}|{rng_id}"))
...
def show_edit_group_menu(chat_id, grp_id, message_id=None):
    ...
    markup = InlineKeyboardMarkup(row_width=1)
    for idx, btn in enumerate(grp.get("buttons", [])): markup.add(InlineKeyboardButton(f"❌ {btn['name']}", callback_data=f"delgrpbtn_{grp_id}_{idx}"))
...
def start_cmd(m):
    ...
    if not check_force_join(uid):
        markup = InlineKeyboardMarkup()
        for link in data.get("force_join_channels", []): markup.add(InlineKeyboardButton("📢 Join Channel", url=link))
    ...
    welcome_text = f"{emo('hi')} <b>Welcome to THE WHITE X Bot!</b>\n\n🔐 Get virtual numbers & receive OTPs instantly\n📊 Check live traffic for any service\n\nUse the buttons below to get started."
...
def help_cmd(m):
    text = f"{emo('info')} <b>BOT COMMANDS</b>\n━━━━━━━━━━━━━━━━━━\n📱 <b>GET NUMBER</b> — Get a virtual number\n📊 <b>TRAFFIC</b> — Check live OTP traffic\n📋 <b>MY HISTORY</b> — View your OTP history\n🔁 <b>New Number 🔁</b> — Last range থেকে নতুন number নাও\n/start — Restart the bot\n/help — Show this help\n/cancel — Cancel current operation\n━━━━━━━━━━━━━━━━━━\n💡 <b>Tips:</b>\n• Use Custom Search to enter any range manually\n• Traffic Check shows which ranges have active OTPs\n• New Number 🔁 instantly reuses your last range!"
...
def text_handler(m):
    ...
    if t == "📱 GET NUMBER": show_user_services(m.chat.id)
        ...
        sent = bot.send_message(m.chat.id, f"{emo('waiting')} <b>Last range থেকে নতুন number নেওয়া হচ্ছে...</b>\n📡 Range: <code>{html.escape(str(last_info.get('range', '')))}</code>", parse_mode="HTML")
...
def query_handler(c):
    ...
    ADMIN_CALLBACKS = {"admin_broadcast", "admin_group_settings", "admin_force_join", "toggle_force_join", "add_fjc", "back_to_admin", "admin_manage_service", "set_main_otp_link", "del_main_otp_link", "add_fwd_group"}
    ...
    if c.data == "back_to_user_services":
        ...
        try: bot.answer_callback_query(c.id, text=f"📋 {num}", show_alert=True)
        except: pass
...
def run_broadcast(chat_id, original_message, msg_id):
    ...
    report = f"{emo('mega')} <b>Broadcast Complete!</b>\n━━━━━━━━━━━━━━━━━━\n{emo('done')} Sent:   <code>{success}</code>\n{emo('cross')} Failed: <code>{failed}</code>\nTotal:  <code>{len(users)}</code>"
...
# ============================================================
#  Auto Live Range Updater (Background Task with Group Forward)
# ============================================================
def auto_update_ranges_worker():
    ...
    while True:
        try:
            ...
            response = requests.get(f"{active_panel['url']}/api/v1/console/logs?limit=200", headers=headers, timeout=15).json()

            if response.get("success") and response.get("data"):
                live_ranges = defaultdict(lambda: defaultdict(set))
                current_ranges_flat = set()
                new_ranges_msg_lines = [] 
                ...
                for item in response["data"]:
                    ...
                    if detected in TARGET_SERVICES and len(num) > 6:
                        rng_pattern = num[:6] + "XXX"
                        live_ranges[detected][country].add(rng_pattern)
                        
                        flat_key = f"{detected}|{country}|{rng_pattern}"
                        current_ranges_flat.add(flat_key)
                        
                        if flat_key not in previous_ranges:
                            new_ranges_msg_lines.append(f"{emo(detected)} <b>{detected.title()}</b> - {get_flag(country)} {country}: <code>{rng_pattern}</code>")
                            previous_ranges.add(flat_key)

                dead_ranges = previous_ranges - current_ranges_flat
                for dead in dead_ranges:
                    previous_ranges.discard(dead)

                if new_ranges_msg_lines:
                    data = load_data()
                    fwd_groups = data.get("forward_groups", [])
                    
                    msg_text = f"{emo('live')} <b>NEW LIVE TRAFFIC DETECTED</b> {emo('live')}\n━━━━━━━━━━━━━━━━━━\n"
                    
                    msg_text += "\n".join(new_ranges_msg_lines[:15])
                    if len(new_ranges_msg_lines) > 15:
                        msg_text += f"\n<i>...and {len(new_ranges_msg_lines)-15} more active ranges!</i>"
                    
                    msg_text += "\n━━━━━━━━━━━━━━━━━━\n🤖 <i>Auto updated in bot</i>"

                    for grp in fwd_groups:
                        try:
                            grp_id_fwd = grp.get("chat_id")
                            if grp_id_fwd:
                                bot.send_message(int(grp_id_fwd), msg_text, parse_mode="HTML")
                        except Exception as fwd_err:
                            print(f"❌ Failed to forward new ranges to group: {fwd_err}")
                ...
                if live_ranges:
                    ...
                    srv_map = {}
                    for srv_id, srv in services_data.items():
                        srv_map[srv["name"].lower()] = srv_id
                    ...
                    for srv_name, countries in live_ranges.items():
                        srv_id = srv_map[srv_name]
                        srv_obj = services_data[srv_id]
                        cnt_map = {cnt["name"].lower(): cnt_id for cnt_id, cnt in srv_obj.get("countries", {}).items()}

                        for cnt_id in list(srv_obj["countries"].keys()):
                            srv_obj["countries"][cnt_id]["ranges"] = {}

                        for cnt_name, ranges_set in countries.items():
                            ...
                            for r_val in ranges_set:
                                ...
                                srv_obj["countries"][cnt_id]["ranges"][r_id] = r_val

                    save_data(data)
                    print(f"✅ Auto-updated live ranges for {TARGET_SERVICES}")

        except Exception as e:
            print(f"❌ Auto Update Error: {e}")

        # ১ মিনিট (৬০ সেকেন্ড) পর পর চেক করবে
        time.sleep(60)
...
if __name__ == "__main__":
    bot.remove_webhook()
    time.sleep(1)
    print("👑 ADVANCED BOT MULTI-PANEL v8.2 — LIVE (Controlled Loop & Auto Updater)")
    ...
    # Run Auto Live Range Updater Thread
