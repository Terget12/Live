...
# ============================================================
# Priority-ordered list: (keyword_patterns, service_name)
# Checks lowercase SMS text + app_name
SMS_SERVICE_PATTERNS = [
    ...
    ("instagram", [
        ...
        "instagram code",
        "instagram",
        "gdgcwrwhvm",        # Instagram's fixed SMS signature
    ]),
    # ── Facebook ──
    ("facebook", [
        "votre code facebook",
        ...
        "facebook code",
        "facebook",
        "h**q+fsn",          # partial FB OTP signature fragment
    ]),
    # ── Telegram ──
    ("telegram", [
        "telegram code",
...
def detect_service_from_sms(sms_text: str, app_name: str) -> str:
    """
    Detect service name from SMS content + app_name.
    Returns lowercase service name string, e.g. 'whatsapp', 'instagram', 'facebook'.
    Falls back to cleaned app_name if nothing matches.
    """
...
COUNTRY_CODE_MAP = {
    ...
    "argentina": "AR", "venezuela": "VE", "peru": "PE",
    "chile": "CL", "bolivia": "BO", "ecuador": "EC",
    ...
    "hungary": "HU", "bulgaria": "BG", "greece": "GR",
    "sweden": "SE", "norway": "NO", "denmark": "DK",
    "finland": "FI", "turkey": "TR", "israel": "IL",
    "iran": "IR", "iraq": "IQ", "saudi arabia": "SA",
    "uae": "AE", "kuwait": "KW", "qatar": "QA",
...
# Flag emoji map by country code
FLAG_MAP = {
    "TG": "🇹🇬", "IV": "🇨🇮", "GH": "🇬🇭", "NG": "🇳🇬",
    "SN": "🇸🇳", "CM": "🇨🇲", "BJ": "🇧🇯", "ML": "🇲🇱",
    "GN": "🇬🇳", "BF": "🇧🇫", "NE": "🇳🇪", "CG": "🇨🇬",
...
def get_country_code(name):
    """Return 2-letter country code/abbreviation for display."""
...
def build_group_forward_markup(grp, copy_text_value, bot_link):
    """
    Build InlineKeyboardMarkup for group forward messages.
    Copy button: CopyTextButton if supported (tap = auto-copy).
    If not supported, skip copy button — text is already in <code> in the message.
    Row 1: [📋 Copy] + first custom button (if any)
    Row 2+: remaining custom buttons
    """
    ...
    if HAS_COPY_BTN:
        ...
        # No CopyTextButton — text is already shown as <code> in message body
        # Just add the custom buttons
...
def forward_range_to_groups(fwd_groups, watermark, service_name, country_name, rng_pattern, bot_link):
    """
    Send a single range as an individual message to all forward groups.
    Format matches the screenshot:
      VIP NUMBER CLUB ⚡  <watermark>
      {flag} {code} [+{range}] {service_emoji} {ServiceName} ({Country}) {time}
    """
    ...
    msg_text = (
        ...
        f"{flag} {country_code} <code>+{rng_pattern}</code> {srv_emoji} "
...
def show_user_ranges(chat_id, srv_id, cnt_id, message_id=None):
    ...
    text = (
        ...
        f"Tap a range to allocate a number:"
    )
    safe_send(chat_id, text, markup, message_id)
...
def fetch_number(chat_id, service_info, msg_id, is_custom=False, retry=0):
    ...
    # Build number list — always as <code> in message (tap = copy)
    num_lines = []
    for num_data in final_numbers:
        num_str = f"+{str(num_data['number']).replace('+', '')}"
        num_lines.append(f"📞 <code>{num_str}</code>")

    text   = (
        f"{emo('number')} <b>Allocated {len(final_numbers)} Number(s)</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        + "\n".join(num_lines) + "\n"
        f"━━━━━━━━━━━━━━━━━━\n"
    ...
    for num_data in final_numbers:
        num_str = f"+{str(num_data['number']).replace('+', '')}"
        if HAS_COPY_BTN: markup.add(InlineKeyboardButton(f"📋 Copy {num_str}", copy_text=CopyTextButton(text=num_str)))
        else:            markup.add(InlineKeyboardButton(f"📋 Copy {num_str}", callback_data=f"copyn|{num_str}"))
    ...
    while time.time() - start < OTP_POLL_TIMEOUT:
        ...
        try:
            ...
            if r.get("success") and r.get("otp"):
                ...
                success_text = (
                    f"{flag} {html.escape(service_info['service_name'])} "
                ...
                markup = InlineKeyboardMarkup(row_width=1)
                if HAS_COPY_BTN: markup.add(InlineKeyboardButton(f"📋 Copy OTP: {otp_code}", copy_text=CopyTextButton(text=str(otp_code))))
                else:            markup.add(InlineKeyboardButton(f"📋 Copy OTP: {otp_code}", callback_data=f"copyn|{otp_code}"))
...
def text_handler(m):
    ...
    if t == "📱 GET NUMBER":
        show_user_services(m.chat.id)
    elif t == "📊 TRAFFIC":
        show_traffic_search(m.chat.id)
    elif t == "⚙️ ADMIN PANEL" and is_admin(uid):
        ...
        sent = bot.send_message(
            m.chat.id,
            f"{emo('waiting')} <b>Last range থেকে নতুন number নেওয়া হচ্ছে...</b>\n📡 Range: <code>{html.escape(str(last_info.get('range', '')))}</code>",
            parse_mode="HTML"
        )
...
def query_handler(c):
    ...
    if c.data == "back_to_user_services":
    ...
    elif c.data == "my_history":
        show_my_history(chat_id, user_id, msg_id)

    elif c.data.startswith("usr_s|"):
        show_user_countries(chat_id, c.data.split("|")[1], msg_id)

    elif c.data.startswith("copyn|"):
        num = c.data.split("|", 1)[1]
        try: bot.answer_callback_query(c.id, text="👆 নিচের code tap করুন — copy হবে!")
        except: pass
        try: bot.send_message(chat_id, f"📋 <code>{html.escape(str(num))}</code>", parse_mode="HTML")
        except: pass
        ...
        if msg_obj:
            threading.Thread(target=fetch_number, args=(chat_id, service_info, msg_obj.message_id, is_custom), daemon=True).start()

    elif c.data == "back_to_admin":
        show_admin_panel(chat_id, msg_id)

    elif c.data == "admin_manage_service":
        show_admin_services(chat_id, msg_id)

    elif c.data == "admin_group_settings":
        safe_send(chat_id, f"{emo('link')} <b>GROUP SETTINGS</b>\n━━━━━━━━━━━━━━━━━━\nManage forward groups and OTP link:", get_group_settings_menu(), msg_id)

    elif c.data == "admin_force_join":
    ...
    elif c.data.startswith("adm_s|"):
        show_admin_countries(chat_id, c.data.split("|")[1], msg_id)
    ...
    while True:
        try:
            ...
            if response.get("success") and response.get("data"):
                ...
                # Store tuples (service_name, country_name, range_pattern) for new ranges
                new_range_entries    = []
                ...
                # Remove dead ranges from tracking
                ...
                if new_range_entries:
                    ...
                    for (srv_name, country_name, rng_pattern) in new_range_entries:
                        forward_range_to_groups(
                            fwd_groups, watermark,
                            srv_name, country_name, rng_pattern,
                            bot_link
                        )
                        # Small delay to avoid Telegram flood limits
                        time.sleep(0.4)

                # ── Sync bot service data: add new, remove dead ranges ──
                data          = load_data()
                services_data = data.setdefault("services_data", {})

                # Build service map: service_name_lower → srv_id
                srv_map = {srv["name"].lower(): srv_id for srv_id, srv in services_data.items()}

                # Auto-create WhatsApp / Facebook / Instagram service if missing
                ...
                for target in TARGET_SERVICES:
                    ...
                    # Live ranges for this service: {country_lower: set_of_ranges}
                    live_for_service = {
                        cnt_name.lower(): rng_set
                        for cnt_name, rng_set in live_ranges.get(target, {}).items()
                    }

                    # Build country map
                    cnt_map = {cnt["name"].lower(): cnt_id for cnt_id, cnt in srv_obj.get("countries", {}).items()}

                    # --- ADD new countries + ranges ---
                    ...
                    # --- REMOVE dead ranges (not in current live data) ---
                    for cnt_key, cnt_id in list(cnt_map.items()):
                        ...
                        if not rng_map and cnt_key not in live_for_service:
                            del srv_obj["countries"][cnt_id]
